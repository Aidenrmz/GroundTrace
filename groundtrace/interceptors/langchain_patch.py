...
from groundtrace.interceptors.base import BaseInterceptor
from groundtrace.session_bus import bus
from groundtrace.types import LLMRequestEvent, LLMResponseEvent, VectorQueryEvent, RetrievedChunk
    ...
    def _wrap_generate(self, original: Callable) -> Callable:
        ...
        def wrapper(self_: Any, messages: list[Any], **kwargs: Any) -> Any:
            ...
            try:
                ...
                _logger.debug("GroundTrace: failed to link queries/responses", exc_info=True)

            try:
                bus.record_llm_request(request_event)
            except Exception:
                _logger.debug("GroundTrace: failed to record request", exc_info=True)
            ...
            try:
                ...
                _logger.debug("GroundTrace: failed to record response", exc_info=True)
    ...
    def _wrap_invoke(self, original: Callable) -> Callable:
        ...
        def wrapper(self_: Any, input: str, **kwargs: Any) -> list[Any]:
            ...
            try:
                ...
                _logger.debug("GroundTrace: failed to record retriever query", exc_info=True)
