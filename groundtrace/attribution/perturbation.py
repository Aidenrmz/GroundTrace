"""Perturbation-based attribution for GroundTrace.
    ...
    if _model is None:
        ...
        if not model_cached:
            print(
                ...
                "(~90MB) for attribution scoring; one-time download...\033[0m",
                file=sys.stderr,
            )
...
class PerturbationAttributor:
    ...
    async def compute_lime(
        ...
        # Generate random masks (n_samples x n_chunks binary matrix)
        ...
        # Ridge regression: masks (KxN) -> similarities (K,)
