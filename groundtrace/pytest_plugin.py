"""GroundTrace pytest plugin.

Provides the `groundtrace_session` fixture and `@pytest.mark.min_groundedness`
marker for asserting RAG groundedness in tests without running a server.

Usage:
    # conftest.py
    pytest_plugins = ['groundtrace.pytest_plugin']

    # test_rag.py
    def test_answer_is_grounded(groundtrace_session):
        response = my_rag(query="What is attention?")
        assert groundtrace_session.groundedness >= 0.8

    @pytest.mark.min_groundedness(0.8)
    def test_answer_grounded_via_marker(groundtrace_session):
...
class GroundTraceSession:
    ...
    def groundedness(self) -> float:
        ...
        raise TimeoutError(
            f"GroundTrace attribution did not complete within {_GROUNDEDNESS_TIMEOUT}s. "
...
@pytest.fixture
def groundtrace_session():
    """Fixture that installs GroundTrace interceptors and yields a session.
    ...
    vl_session = GroundTraceSession(session.id, bus)
...
def pytest_runtest_call(item: pytest.Item):
    ...
    session_fixture = item.funcargs.get("groundtrace_session")
    if session_fixture is None:
        raise pytest.UsageError(
            "@pytest.mark.min_groundedness requires the `groundtrace_session` fixture"
        )
