"""Tests for the GroundTrace pytest plugin."""
import pytest

from groundtrace.pytest_plugin import GroundTraceSession
from groundtrace.session_bus import SessionBus
from groundtrace.types import (
    LLMRequestEvent,
    LLMResponseEvent,
    RetrievedChunk,
    VectorQueryEvent,
)
from groundtrace.pipeline import _run_attribution
...
def test_groundtrace_session_groundedness_returns_score():
    """GroundTraceSession.groundedness returns attribution score."""
    bus = SessionBus()
    sid = _make_session_with_attribution(bus, groundedness=1.0)
    vl = GroundTraceSession(sid, bus)
    score = vl.groundedness
    assert 0.0 <= score <= 1.0


def test_groundtrace_session_groundedness_timeout():
    """GroundTraceSession.groundedness raises TimeoutError if no attribution."""
    import groundtrace.pytest_plugin as plugin
    ...
    try:
        bus = SessionBus()
        session = bus.new_session()
        vl = GroundTraceSession(session.id, bus)
...
def test_groundtrace_session_no_state_leakage():
    """Two GroundTraceSessions on separate buses don't share attribution."""
    ...
    vl1 = GroundTraceSession(sid1, bus1)
    vl2 = GroundTraceSession(sid2, bus2)
...
def test_groundtrace_session_attributions_property():
...
def test_groundtrace_session_hallucinated_count():
