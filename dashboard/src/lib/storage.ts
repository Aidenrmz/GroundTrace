...
const STORAGE_KEY = 'groundtrace_sessions';
...
export function saveSessions(sessions: Session[]): void {
  try {
    ...
    const SIZE_LIMIT = 4 * 1024 * 1024; // 4MB, leaving room below common 5MB limits.

    if (serialized.length > SIZE_LIMIT) {
      // Too large: store summaries only.
      ...
      if (summaryStr.length <= SIZE_LIMIT) {
        ...
        console.warn('GroundTrace: sessions too large for localStorage, storing summaries only');
        return;
      }
      // Even summaries are too large; skip storage entirely.
      console.warn('GroundTrace: sessions too large to cache, skipping localStorage');
      return;
    }
